# app/api/v1/endpoints/rag_faiss.py
...
# /api/v1/rag_faiss 아래로 묶이는 라우터 (기존 /rag 대신 구분용)
router = APIRouter(prefix="/rag_faiss", tags=["rag_faiss"])
...
async def rag_generate(body: RagQARequest) -> RagQAResponse:
    """
    업로드된 PDF/문서를 기반으로
    QA 테스트케이스 JSON을 생성하는 일회성 RAG 엔드포인트. (FAISS 버전 유지)
    """
